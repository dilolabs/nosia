...
setup_env() {
  ...
  if ! [ -n "$AI_BASE_URL" ]; then
    ...
    darwin*) AI_BASE_URL=http://host.docker.internal:12434/engines/llama.cpp/v1 ;;
    cygwin* | msys* | win32) AI_BASE_URL=http://host.docker.internal:12434/engines/llama.cpp/v1 ;;
    esac
  fi
